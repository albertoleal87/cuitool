...
function profile() {
	 document.getElementById("left").value = "Haciendo peticion..."
	var xmlhttp;
	if (window.XMLHttpRequest) { // code for IE7+, Firefox, Chrome, Opera, Safari
		xmlhttp = new XMLHttpRequest();
	} else { // code for IE6, IE5
...
function verify() {
	if (okSoap()) {
		document.getElementById("right").innerHTML = "Haciendo peticion..."
		var xmlhttp;
		if (window.XMLHttpRequest) { // code for IE7+, Firefox, Chrome, Opera, Safari
			xmlhttp = new XMLHttpRequest();
		} else { // code for IE6, IE5
		...
		xmlhttp.onreadystatechange = function() {
			if (xmlhttp.readyState==4) {
				if (xmlhttp.status==200 || xmlhttp.status==0) {
					document.getElementById("right").innerHTML = xmlhttp.responseText.replace(/^\n/, "");
				} else {
					alert("Error " + xmlhttp.status);
				}
			}
